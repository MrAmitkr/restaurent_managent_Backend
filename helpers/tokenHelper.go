...
func GenerateAllTokens(email string, firstName string, lastName string, uid string) (signedTokon string, signedRefreshToken string, err error){
...
func UpdateAllTokens(signedToken string signedrefreshToken string, userId string) {
	...
	Updated_at, _ := time.Parse(time.RFC3339, time.Now().Format(time.RFC339)) 
...
func ValidateToken(signedToken string)(claims *SignedDetails, msg string) {
	...
	claims, of := token.Claims.(*SignedDetails)
