...
func GetInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if invoice.Payment_method != nil{
			invoiceView.Payment_mwthod = *invoice.Payment_method
		}

		invoiceView.Invoice_id - invoice.Invoice_id
		invoiceView.Paymnent_status = *&invoice.Payment_status
		invoiceView.Payment_due = allOrderItems[0]["paymnent_due"]
...
func CreateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		invocie.Payment_due_date, _ = time.Parse(time.RFC3339, time.Now().AddDate(0,0,1).Format(time.RFC3339))
		...
		result, insertErrinvocieCollection.InserOne(ctx, invoice)
		if insertErr != nil{
...
func UpdateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if invoice.Payment_status !- nil{
			...
			updateObj = append(updteObj bson.E{"update_at", invoice.Payment_status})
